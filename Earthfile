...
integration-test-mysql:
    ...
    WITH DOCKER \
        ...
        RUN set -e; \
            timeout=$(expr $(date +%s) + 30); \
            docker run --name mysql --network=host -e MYSQL_ROOT_PASSWORD=root "mysql:$MYSQL" --sql_mode="ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION,ANSI_QUOTES"; \
